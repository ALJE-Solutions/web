...
- **2026-06-18**: Nuevo componente reutilizable `src/components/WaveDivider.astro` (props `topColor`/`bottomColor`) para los separadores en forma de ola entre secciones de distinto color, responsive (70px desktop, 48px ≤768px, 32px ≤480px). Aplicado en las 7 transiciones de color de `index.astro` (hero→sectores→servicios→packs→nosotros→proceso→trabajos→contacto) y en las 2 de `casos-de-exito.astro`. De paso se corrigieron dos bugs de las olas que ya existían entre Nosotros y Proceso: la primera (antes de Nosotros) tenía el fondo del contenedor mal (blanco fijo en vez de gris `#f5f7fa`, el color real de la sección Packs justo encima); la segunda (después de Nosotros) tenía el `fill` del SVG con el mismo color que el fondo del contenedor, por lo que la ola era completamente invisible. Verificado visualmente con capturas en desktop y móvil (375-390px) en home y casos-de-éxito; sin overflow horizontal.
- **2026-06-18**: Ola añadida también antes del footer (siempre oscuro). Como el footer es compartido por `Layout.astro` en todas las páginas, se añadió la prop `footerTopColor` a `Layout.astro` (default `#ffffff`, el color con el que termina la mayoría de páginas) que `casos-de-exito.astro` sobreescribe a `#1c1c2e` al ser la única página que termina en sección oscura. Además, en `WaveDivider.astro` se añadió un solape de `margin: -1px 0` entre el divisor y las secciones vecinas para evitar un hairline (línea fina del color de fondo del propio div) que puede aparecer en algunos navegadores/zooms por redondeo de subpíxeles en el borde del SVG.
- **2026-06-18**: Fix de bug responsive en la sección "¿Quiénes somos?" (`#nosotros` en `index.astro`): cada `.about-card` tenía el icono, el `<h3>` y el `<p>` como hijos flex directos en fila (sin `flex-wrap` ni contenedor de texto), así que en pantallas estrechas el título y el párrafo se ponían lado a lado en vez de uno debajo del otro y desbordaban horizontalmente el contenedor (min-content por defecto de los flex/grid items). Arreglado envolviendo `<h3>`+`<p>` en un nuevo `<div class="about-text">` con `flex:1; min-width:0`, y añadiendo `min-width:0` también a `.about-card`. Verificado sin overflow horizontal en 320/375/768/1400px.
